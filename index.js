...
// ── 5. 마감 3일전 미응답 회원 리마인더 (매일 오전 9시 EST) ────────
...
// ── 6. 공지사항 등록 시 푸시 알림 ────────────────────────────
exports.onNewNotice = onDocumentCreated("notices/{noticeId}", async (event) => {
  const data = event.data.data();
  if (!data) return;
  ...
  const message = {
    notification: {
      title: "📢 새 공지사항",
      body: `${data.author}: ${data.content.slice(0, 60)}${data.content.length > 60 ? '...' : ''}`
    },
    data: { tag: "new_notice" },
    tokens: tokens
  };
  ...
  try {
    const response = await admin.messaging().sendEachForMulticast(message);
    console.log(`공지 푸시: 성공 ${response.successCount}, 실패 ${response.failureCount}`);
  } catch (e) {
    console.error("공지 푸시 오류:", e);
  }
});
